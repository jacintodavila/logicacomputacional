...
sr([cierto|X],['sí'|Y],X,Y).
sr(['quizás'|X],[puede,ser|Y],X,Y).
sr(['alemán'|X],[xfremd|Y],X,Y).
sr(['francés'|X],[xfremd|Y],X,Y).
sr(['inglés'|X],[xfremd|Y],X,Y).
sr(['máquina'|X],[computadora|Y],X,Y).
sr(['máquinas'|X],[computadora|Y],X,Y).
sr([computadoras|X],[computer|Y],X,Y).
sr([tuyo|X],['mío'|Y],X,Y).
sr([tuyos|X],['míos'|Y],X,Y).
...
rules([[computadora,50],[
        [1,[_],0,
                [tanto,te,ladilla,que,sea,una,' máquina' ,?],
                [y,vuelves,con,la,misma,tonteria,de,la,computadora,?],
                [crees,que,me,puedes,fregar,solo,porque,sea,una,' máquina' ,?],
